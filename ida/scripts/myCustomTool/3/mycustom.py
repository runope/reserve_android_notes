...
def main():
    ...
    for ea_offset in range(rodata_ea_start, rodata_ea_end):
        cur_string = str(idc.get_strlit_contents(ea_offset))
    ...
    # xref to JNI_OnLoad...
    refs = idautils.XrefsTo(string_jni_on_load_ea)
    useful_ref = 0
    for ref in refs:
        useful_ref = ref.frm
        break
    ...
    func_items = idautils.FuncItems(useful_ref)
    ...
    func_items = idautils.FuncItems(target_address)
